        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-lg bg-[#1A2236] p-4">
              ...
              <p className="mt-1 font-mono text-xl font-bold text-[#F9FAFB]">{stat.value}</p>
              <p className="mt-1 text-xs text-[#9CA3AF]">{stat.detail}</p>
            </div>
          ))}
        </div>
      )}
          ...
          {expanded && (
            <div className="mt-4 border-t border-[#1E2D45] pt-4">
              {Array.isArray(fullSummary) ? (
                ...
                <p className="leading-relaxed text-[#D1D5DB]">{fullSummary}</p>
              )}
            </div>
          )}
        </>
